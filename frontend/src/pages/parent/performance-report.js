...
const ParentPerformanceReport = () => {
  ...
  const fetchReport = async (studentId) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:5000/api/performancereports/details/${studentId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        {report ? (
          <div>
            <h3 className="text-xl font-medium mb-4">
              Report for Student: {getStudentNameById(report.studentId._id)}
            </h3>
            <p className="mb-4">
              <span className="font-semibold">Final Average:</span>{" "}
              {report.finalAverage !== undefined ? report.finalAverage : "N/A"}
            </p>
            <p className="mb-4">
              <span className="font-semibold">Passed Subjects:</span>{" "}
              {report.passedSubjects !== undefined
                ? report.passedSubjects
                : "N/A"}
            </p>
            <p className="mb-4">
              <span className="font-semibold">Failed Subjects:</span>{" "}
              {report.failedSubjects !== undefined
                ? report.failedSubjects
                : "N/A"}
            </p>
            <p className="mb-4">
              <span className="font-semibold">Total Subjects:</span>{" "}
              {report.totalSubjects !== undefined
                ? report.totalSubjects
                : "N/A"}
            </p>
            <div className="mb-4">
              <span className="font-semibold">Trimesters:</span>
              {report.trimesters && report.trimesters.length > 0 ? (
                <ul>
                  {report.trimesters.map((trimester) => (
                    <li key={trimester._id}>
                      <strong>{trimester.trimester}:</strong> Average:{" "}
                      {trimester.average}
                    </li>
                  ))}
                </ul>
              ) : (
                "No trimesters data available."
              )}
            </div>
            <p className="mb-4">
...
export default ParentPerformanceReport;
